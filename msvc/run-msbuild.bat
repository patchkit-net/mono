...
:: %4 Mono GC, sgen|boehm, default sgen.
:: %5 MSVC platform toolset, v140|v141, default v140
:: %6 Optional additional arguments passed to msbuild, needs to be quoted if multiple.
:: -------------------------------------------------------
...
if /i "%~1" == "v141" (
    set VS_PLATFORM_TOOLSET="v141"
)
shift

set VS_CUSTOM_ARGUMENTS=
if /i not "%~1" == "" (
    set VS_CUSTOM_ARGUMENTS=%~1
)
...
set VS_BUILD_ARGS=/p:PlatformToolset=%VS_PLATFORM_TOOLSET% /p:Configuration=%VS_CONFIGURATION% /p:Platform=%VS_PLATFORM% /p:MONO_TARGET_GC=%MONO_GC% /t:%VS_TARGET% %VS_CUSTOM_ARGUMENTS%
