...
export CI_TAGS="win-amd64,winaot_llvm,helix-telemetry"
...
if [[ ${CI_TAGS} == *'win-i386'* ]];
    ...
    ${TESTCMD} --label=make-msvc --timeout=60m --fatal ./msvc/run-msbuild.sh "build" "${PLATFORM}" "release" "boehm" "v140" "${MSBUILD_CXX}"
fi
if [[ ${CI_TAGS} == *'win-'* ]];
    then
    ${TESTCMD} --label=make-msvc-sgen --timeout=60m --fatal ./msvc/run-msbuild.sh "build" "${PLATFORM}" "release" "sgen" "v140" "${MSBUILD_CXX}"
fi
